...
state = {
    "version": "1.10 COIN FIRST FAST CONTEXT",
...
# v1.10 — COIN FIRST architecture. The coin context is calculated continuously from
...
def _structural_stop(side, entry, df, atr, coin_invalidation=None):
    ...
    if side=="BUY":
        ...
        risk=stop-entry
    # Coin-first invalidation is the primary logical stop when available.
    if coin_invalidation is not None:
        ci=float(coin_invalidation)
        if side=="BUY" and ci<entry:
            stop=min(stop,ci)
        elif side=="SELL" and ci>entry:
            stop=max(stop,ci)
        risk=abs(entry-stop)
    if risk<=0 or risk>min(MAX_STOP_RISK_ATR,COIN_STOP_MAX_ATR)*atr:
        return None
    return float(stop)
...
def _tf_structure_score(df):
    """Return BUY/SELL structure evidence from the latest available live candle."""
    if df is None or len(df) < 12:
        return 0.0, 0.0, []
    x=df.iloc[-1]; recent=df.iloc[-7:]
    if any(pd.isna(v) for v in [x.ema20,x.ema50,x.atr]):
        return 0.0,0.0,[]
    buy=sell=0.0; notes=[]
    if x.close >= x.ema20: buy+=12
    else: sell+=12
    if x.ema20 >= x.ema50: buy+=13
    else: sell+=13
    # Live swing progression, not closed-pivot confirmation.
    lows=recent.low.values; highs=recent.high.values
    if len(lows)>=4:
        if lows[-1] >= min(lows[-4:-1]): buy+=7
        else: sell+=4
        if highs[-1] >= max(highs[-4:-1]): buy+=6
        else: sell+=3
    if x.close >= x.vwap if pd.notna(x.vwap) else False: buy+=5
    elif pd.notna(x.vwap): sell+=5
    if buy>sell+8: notes.append("بنية صاعدة")
    elif sell>buy+8: notes.append("بنية هابطة")
    else: notes.append("بنية متوازنة")
    return buy,sell,notes


def build_coin_context(symbol, dfs):
    """Analyze the coin FIRST. No extra REST calls and no candle-close waiting."""
    buy=sell=0.0; reasons_buy=[]; reasons_sell=[]
    weights={"4h":1.00,"1h":0.85,"15m":0.65}
    for tf,w in weights.items():
        df=dfs.get(tf)
        if df is None: continue
        b,ss,notes=_tf_structure_score(df)
        buy += b*w; sell += ss*w
        if notes:
            reasons_buy.append(f"{tf.upper()} {notes[0]}")
            reasons_sell.append(f"{tf.upper()} {notes[0]}")

    d15=dfs.get("15m") if dfs.get("15m") is not None else dfs.get("1h")
    invalid_buy=invalid_sell=None
    location="غير محدد"
    helpers_buy=[]; helpers_sell=[]
    if d15 is not None and len(d15)>=12:
        x=d15.iloc[-1]; p=d15.iloc[-2]; atr=max(float(x.atr) if pd.notna(x.atr) else abs(float(x.close))*0.01,1e-12)
        recent=d15.iloc[-10:]
        local_low=float(recent.low.min()); local_high=float(recent.high.max())
        invalid_buy=local_low-COIN_STOP_BUFFER_ATR*atr
        invalid_sell=local_high+COIN_STOP_BUFFER_ATR*atr
        # Location score: reward entries near meaningful value/liquidity, not mid-extension.
        near_low=(float(x.close)-local_low)/atr <= 0.80
        near_high=(local_high-float(x.close))/atr <= 0.80
        near_vwap= pd.notna(x.vwap) and abs(float(x.close)-float(x.vwap))/atr <= 0.55
        near_ema= pd.notna(x.ema20) and abs(float(x.close)-float(x.ema20))/atr <= 0.55
        if near_low: buy+=16; reasons_buy.append("قريب من سيولة/قاع محلي")
        if near_high: sell+=16; reasons_sell.append("قريب من سيولة/قمة محلية")
        if near_vwap or near_ema:
            buy+=6; sell+=6; location="منطقة قيمة/إعادة اختبار"
        else:
            location="قرب طرف النطاق" if (near_low or near_high) else "منتصف الحركة"
        # Live flow/helpers are supporting evidence, never mandatory gates.
        if x.delta>0: buy+=6; helpers_buy.append("Delta")
        elif x.delta<0: sell+=6; helpers_sell.append("Delta")
        if x.cvd>p.cvd: buy+=6; helpers_buy.append("CVD")
        elif x.cvd<p.cvd: sell+=6; helpers_sell.append("CVD")
        if pd.notna(x.macd) and pd.notna(x.macd_signal):
            if x.macd>=x.macd_signal: buy+=4; helpers_buy.append("MACD")
            else: sell+=4; helpers_sell.append("MACD")
        if pd.notna(x.vol_ma) and x.volume>1.25*x.vol_ma:
            (helpers_buy if x.close>=x.open else helpers_sell).append("VolumeSpike")
            if x.close>=x.open: buy+=4
            else: sell+=4

    # Normalize gently; both sides may score in ranges, which is intended.
    buy=max(0.0,min(100.0,buy)); sell=max(0.0,min(100.0,sell))
    preferred="BUY" if buy>=sell+6 else "SELL" if sell>=buy+6 else "NEUTRAL"
    ctx={
        "symbol":symbol,"ts":time.time(),"buy_score":round(buy,1),"sell_score":round(sell,1),
        "preferred":preferred,"location":location,"invalid_buy":invalid_buy,"invalid_sell":invalid_sell,
        "reasons_buy":reasons_buy[-5:],"reasons_sell":reasons_sell[-5:],
        "helpers_buy":list(dict.fromkeys(helpers_buy)),"helpers_sell":list(dict.fromkeys(helpers_sell)),
    }
    with coin_context_lock:
        coin_context_cache[symbol]=ctx
    return ctx
...
def context_allows_signal(ctx, s):
    ...
    # Never make context excessively strict. Strong setups may pass a neutral market,
    # but a clearly opposing coin analysis blocks the trade before strategy confirmation.
    allowed = own >= COIN_CONTEXT_MIN_SCORE and not (opp>=COIN_CONTEXT_MAX_OPPOSITION and opp>own+12)
    return allowed,own,opp
...
def attach_coin_context(s, ctx):
    ...
    own=float(ctx.get("buy_score" if side=="BUY" else "sell_score",0))
    opp=float(ctx.get("sell_score" if side=="BUY" else "buy_score",0))
    s["coin_context"]={"score":own,"opposition":opp,"preferred":ctx.get("preferred"),"location":ctx.get("location")}
    s["context_score"]=own
    inv=ctx.get("invalid_buy" if side=="BUY" else "invalid_sell")
    if inv is not None: s["coin_invalidation"]=float(inv)
    creasons=ctx.get("reasons_buy" if side=="BUY" else "reasons_sell",[])
    s["reasons"]=list(s.get("reasons",[]))+[f"تحليل العملة أولًا: {own:.0f}%", f"موقع السعر: {ctx.get('location','-')}"]+list(creasons[:2])
    chelp=ctx.get("helpers_buy" if side=="BUY" else "helpers_sell",[])
    s["helpers"]=list(dict.fromkeys(list(s.get("helpers",[]))+list(chelp)))
    return s


def _fast_live_context_confirmation(p, df, price):
    """First live confirmation only: no extra candle and no forced break+retest cycle."""
    if not FAST_CONTEXT_CONFIRM or df is None or len(df)<4 or price is None:
        return None
    s=p.get("setup",{}); ctx=s.get("coin_context") or {}
    score=float(ctx.get("score",0))
    if score < FAST_CONFIRM_MIN_SCORE:
        return None
    x=df.iloc[-1]; prev=df.iloc[-2]
    if pd.isna(x.atr): return None
    atr=max(float(x.atr),1e-12); entry0=float(p.get("setup_entry",price))
    if abs(float(price)-entry0)/atr > FAST_CONFIRM_MAX_DISTANCE_ATR:
        return None
    side=p["side"]
    bull=side=="BUY"
    # Two-of-four live micro confirmations; none requires candle close.
    votes=0
    votes += int((float(price)>=float(x.open)) if bull else (float(price)<=float(x.open)))
    votes += int((float(x.delta)>0) if bull else (float(x.delta)<0))
    votes += int((float(x.cvd)>=float(prev.cvd)) if bull else (float(x.cvd)<=float(prev.cvd)))
    if pd.notna(x.vwap): votes += int((float(price)>=float(x.vwap)) if bull else (float(price)<=float(x.vwap)))
    if votes>=2:
        return {"confirmed":True,"entry":float(price),"atr":atr,"bar_open_time":int(x.open_time),"mode":"first live context confirmation","fast":True}
    return None
...
def alert_text(symbol,s,helpers_list,oi,funding,combo=None):
    ...
    ctx_txt=f"🧭 تحليل العملة: <b>{float(ctx.get('score',0)):.0f}%</b> | الموقع: <b>{ctx.get('location','-')}</b>" if ctx else "🧭 تحليل العملة: غير متاح"
...
def analyze_symbol(symbol):
    ...
    # 2) Strategy is a matcher/trigger inside the already-known coin context.
    candidates=[]
    ...
    # 3) Keep only strategies that fit the coin analysis. Helpers add confidence, not delay.
