...
class Engine:
    ...
    async def start(self) -> None:
        ...
        if SEND_STARTUP_MESSAGE and BOT_TOKEN and CHAT_ID:
            await send_telegram(
                self.telegram,
                "✅ <b>Ahmed Early Explosion Trader v2.1 BLUE-ENTRY بدأ العمل</b>\n\n"
            ...
            async with sem:
                try:
                    ...
                    for tf in TIMEFRAMES:
                        ...
                        persistent_state = await get_alert_state(symbol, tf)
                        if (
                            persistent_state
                            and persistent_state.get("direction") == direction
                            and persistent_state.get("sent_early")
                            and not persistent_state.get("sent_entry")
                            and persistent_state.get("early_price") is not None
                            and persistent_state.get("early_atr")
                            and score >= EARLY_TO_ENTRY_MIN_SCORE
                        ):
                            early_price = float(persistent_state["early_price"])
                            early_atr = float(persistent_state["early_atr"])
                            signed_move_atr = (
                                (float(cf["price"]) - early_price) / early_atr
                                if direction == "BUY"
                                else (early_price - float(cf["price"])) / early_atr
                            )
                            trigger = (
                                EARLY_TO_ENTRY_MOVE_ATR_1H
                                if tf == "1h"
                                else EARLY_TO_ENTRY_MOVE_ATR_4H
                            )
                            if signed_move_atr >= trigger:
                                stage = "ENTRY"
...
async def health():
    return {
        ...
        "service": "Ahmed Early Explosion Trader v2.1 BLUE-ENTRY",
...
<body><div class="wrap">
<h1>Ahmed Early Explosion Trader v2.1 BLUE-ENTRY</h1>
