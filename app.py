...
INTERNAL_TFS = list(dict.fromkeys(ENABLED_TFS + ["5m"]))
...
CONFIRM_TF = os.getenv("CONFIRM_TIMEFRAME", "5m").strip().lower()
CONFIRM_SWING_BARS = int(os.getenv("CONFIRM_SWING_BARS", "6"))
CONFIRM_RETEST_ATR = float(os.getenv("CONFIRM_RETEST_ATR", "0.20"))
CONFIRM_MAX_EXTENSION_ATR = float(os.getenv("CONFIRM_MAX_EXTENSION_ATR", "0.85"))
...
def _setup_invalidation_hit(p, price):
    return price <= p["sl"] if p["side"]=="BUY" else price >= p["sl"]
    ...
    with pending_lock:
        for s in signals:
            ...
            pending_confirmations[key]={
                ...
                "setup_entry":float(s["entry"]), "sl":float(s["sl"]),
            }

def _confirmation_df(symbol):
    # Prefer 5m internal confirmation; fall back to 15m if unavailable.
    df=df_from_store(symbol,CONFIRM_TF)
    if df is None or len(df)<max(20,CONFIRM_SWING_BARS+8):
        df=df_from_store(symbol,"15m")
    return df

def _structure_confirmation(p, df):
    """Price/structure first: break opposing micro structure, then hold/retest it.
    Helpers may strengthen the alert but never replace structural confirmation.
    """
    if df is None or len(df)<max(20,CONFIRM_SWING_BARS+8): return None
    x=df.iloc[-1]; prev=df.iloc[-(CONFIRM_SWING_BARS+2):-2]
    if prev.empty or pd.isna(x.atr): return None
    side=p["side"]; atr=float(x.atr)
    if side=="BUY":
        level=float(prev.high.max())  # last opposing lower-high area
        broke=float(x.high)>level and float(x.close)>level
        hold=float(x.low)>=level-CONFIRM_RETEST_ATR*atr and float(x.close)>level
        candle_ok=float(x.close)>float(x.open)
        invalid=float(x.close)<=float(p["sl"])
        extension=(float(x.close)-level)/atr
    else:
        level=float(prev.low.min())   # last opposing higher-low area
        broke=float(x.low)<level and float(x.close)<level
        hold=float(x.high)<=level+CONFIRM_RETEST_ATR*atr and float(x.close)<level
        candle_ok=float(x.close)<float(x.open)
        invalid=float(x.close)>=float(p["sl"])
        extension=(level-float(x.close))/atr
    if invalid: return {"cancel":True,"reason":"setup invalidated"}
    # Do not chase a move that already expanded too far before confirmation.
    if extension > CONFIRM_MAX_EXTENSION_ATR:
        return {"cancel":True,"reason":"confirmation arrived too late / extended"}
    if broke and hold and candle_ok:
        return {"confirmed":True,"level":level,"entry":float(x.close),"atr":atr}
    return None

def evaluate_waiting_confirmations(symbol):
    now=time.time(); price=mark_price_cache.get(symbol)
    df=_confirmation_df(symbol)
    confirmed=[]; remove=[]
    with pending_lock:
    ...
    for key,p in items:
        ...
        if price and _setup_invalidation_hit(p,float(price)):
            remove.append(key); continue
        res=_structure_confirmation(p,df)
        if res and res.get("cancel"):
            remove.append(key); continue
        if res and res.get("confirmed"):
            s=dict(p["setup"])
            # Entry is the actual confirmation price. Keep the strategy's original invalidation logic.
            s["entry"]=float(res["entry"])
            risk=abs(float(s["entry"])-float(s["sl"]))
            if risk<=0:
                remove.append(key); continue
            s["tp"]=[s["entry"]+(risk*r if s["side"]=="BUY" else -risk*r) for r in (1,2,3)]
            s["quality"]=min(99,float(s.get("quality",0))+4)
            s["reasons"]=list(s.get("reasons",[]))+[
                f"تأكيد هيكلي على {CONFIRM_TF.upper() if CONFIRM_TF else '5M'}",
                "كسر البنية/البلوك المعاكس",
                "ثبات السعر بعد الكسر"
            ]
