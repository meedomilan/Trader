...
state = {
    "version": "1.17 REAL AGGTRADE FOOTPRINT",
...
def alert_text(symbol,s,helpers_list,oi,funding,combo=None):
    ...
    if combo: head="💎🔥🟢 تنبيه دمج مميز — توافق استراتيجيات — شراء" if buy else "💎🔥🔴 تنبيه دمج مميز — توافق استراتيجيات — بيع"
    strat=" + ".join(combo) if combo else s["strategy"]
    reasons="\n".join("✅ "+r for r in list(dict.fromkeys(s.get("reasons",[]))))
    ...
    if ctx:
        ...
        t1=_direction_ar("1h",ctx.get("trend_1h","NEUTRAL")); t4=_direction_ar("4h",ctx.get("trend_4h","NEUTRAL")); td=_direction_ar("1d",ctx.get("trend_1d","NEUTRAL")); tw=_direction_ar("1w",ctx.get("trend_1w","NEUTRAL"))
        ctx_txt=(f"🧠 قرار المحلل: <b>{ctx.get('decision','WAIT')}</b> | الثقة: <b>{float(ctx.get('score',0)):.0f}%</b>\n"
                 f"🧭 الاتجاه العام: <b>{t1}</b> | <b>{t4}</b> | <b>{td}</b> | <b>{tw}</b>\n"
                 f"🗺 البنية: 4H {ctx.get('market_4h','-')} | 1H {ctx.get('market_1h','-')} | 15M {ctx.get('market_15m','-')}\n"
                 f"📍 {ctx.get('location','-')} | منطقة الخطة: <b>{zone_txt}</b> | مساحة الهدف: {ctx.get('rr_space','-')}R")
    else:
        ctx_txt="🧠 تحليل العملة: غير متاح"
    of_blocks=s.get("vol_of_blocks") or []
    if of_blocks:
        parts=[]
        for b in of_blocks[:3]:
            typ="🟩 بلوك شرائي قوي" if b.get("side")=="BUY" else "🟥 بلوك بيعي قوي"
            parts.append(f"{typ} — REAL aggTrade Footprint — <b>{b.get('tf','').upper()}</b> — قوة <b>{float(b.get('score',0)):.0f}%</b> | Δ <b>{float(b.get('delta_pct',0)):+.1f}%</b> | POC <b>{fmt_price(float(b.get('poc',0)))}</b>")
        of_txt="\n".join(parts)
    else:
        of_txt="▫️ REAL aggTrade Footprint: لا يوجد بلوك قوي مكتمل/قريب من الدخول بعد"
...
def _setup_entry_gate(ctx, sig):
    ...
    trends=[ctx.get("trend_1h","NEUTRAL"),ctx.get("trend_4h","NEUTRAL"),ctx.get("trend_1d","NEUTRAL"),ctx.get("trend_1w","NEUTRAL")]
    aligned=sum(1 for x in trends if x==want)
    if ctx.get("trend_1h")!=want or ctx.get("trend_4h")!=want:
        return False, "اتجاه 1H و4H غير متوافق مع الصفقة"
    if aligned < MTF_TREND_MIN_ALIGNED:
        return False, f"توافق الاتجاه العام غير كافٍ ({aligned}/4)"
