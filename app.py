...
EARLY_TO_ENTRY_MOVE_ATR_1H = float(os.getenv("EARLY_TO_ENTRY_MOVE_ATR_1H", "0.22"))
EARLY_TO_ENTRY_MOVE_ATR_4H = float(os.getenv("EARLY_TO_ENTRY_MOVE_ATR_4H", "0.18"))
EARLY_TO_ENTRY_MIN_SCORE = float(os.getenv("EARLY_TO_ENTRY_MIN_SCORE", "66"))

TIMING_INTERVALS = {"1h": "5m", "4h": "15m"}
INTENT_THRESHOLD = float(os.getenv("INTENT_THRESHOLD", "54"))
ENTRY_FLOW_THRESHOLD = float(os.getenv("ENTRY_FLOW_THRESHOLD", "58"))
EXPLOSION_FLOW_THRESHOLD = float(os.getenv("EXPLOSION_FLOW_THRESHOLD", "72"))
MAX_EARLY_EXTENSION_ATR = float(os.getenv("MAX_EARLY_EXTENSION_ATR", "0.35"))
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        columns = {
            row[1] for row in await (
                await db.execute("PRAGMA table_info(alert_states)")
            ).fetchall()
        }
        if "early_price" not in columns:
            await db.execute("ALTER TABLE alert_states ADD COLUMN early_price REAL")
        if "early_atr" not in columns:
            await db.execute("ALTER TABLE alert_states ADD COLUMN early_atr REAL")
        await db.commit()
...
async def claim_stage_once(
    ...
    async with aiosqlite.connect(DB_PATH) as db:
            ...
            await db.execute(
                """INSERT INTO alert_states
                   (symbol,timeframe,direction,sent_early,sent_entry,
                    sent_explosion,sent_weakening,below_count,last_score,
                    early_price,early_atr,updated_at)
                   VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""",
                (
                    ...
                    0, score,
                    price if stage == "EARLY" else None,
                    atr_value if stage == "EARLY" else None,
                    ts,
                ),
            )
            await db.commit()
        ...
        # لا نعكس الاتجاه داخل نفس الدورة. يبدأ اتجاه جديد فقط بعد حذف
        # الحالة بواسطة ثلاثة فحوص هادئة متتالية.
        ...
        if stage == "EARLY":
            await db.execute(
                f"""UPDATE alert_states
                    SET {column}=1,last_score=?,below_count=0,
                        early_price=?,early_atr=?,updated_at=?
                    WHERE symbol=? AND timeframe=?""",
                (score, price, atr_value, ts, symbol, timeframe),
            )
        else:
            await db.execute(
                f"""UPDATE alert_states
...
async def get_alert_state(symbol: str, timeframe: str) -> dict[str, Any] | None:
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        row = await (
            await db.execute(
...
def timing_gate(
    ...
    if stage == "ENTRY":
        if age > 1:
            return False, f"فات الدخول؛ عمر الكسر {age} شمعات"
...
def micro_flow_features(rows: list[list[Any]]) -> dict[str, float | bool]:
    opens = [float(x[1]) for x in rows]
    highs = [float(x[2]) for x in rows]
    lows = [float(x[3]) for x in rows]
    closes = [float(x[4]) for x in rows]
    ...
    buy_q = sum(taker_buy_quote[-3:])
    total_q = max(sum(quote_volumes[-3:]), 1.0)
    delta = safe_div(2 * buy_q - total_q, total_q) * 100

    avg_q = max(sum(quote_volumes[-23:-3]) / 20, 1.0)
    volume_ratio = quote_volumes[-1] / avg_q
    volume_accel = quote_volumes[-1] / max(quote_volumes[-2], 1.0) - 1.0

    prior_high = max(highs[-7:-1])
    prior_low = min(lows[-7:-1])
    micro_break_up = price > prior_high
    micro_break_down = price < prior_low

    body_atr = safe_div(closes[-1] - opens[-1], a)
    move_atr = safe_div(closes[-1] - closes[-3], a)
    extension_up = safe_div(price - prior_high, a) if price > prior_high else 0.0
    extension_down = safe_div(prior_low - price, a) if price < prior_low else 0.0
...
def intent_score(direction: str, base: dict[str, float | bool], micro: dict[str, float | bool]) -> tuple[float, list[str]]:
    sign = 1 if direction == "BUY" else -1
    score = 0.0
    reasons: list[str] = []

    def add(points: float, ok: bool, label: str) -> None:
        nonlocal score
        if ok:
            score += points
            reasons.append(label)

    # نية الحركة: التدفقات أهم من حركة السعر نفسها.
    add(20, sign * float(base["oi_change"]) >= 0.04, "العقود المفتوحة بدأت تدعم الاتجاه")
    add(18, sign * float(base["cvd_bias"]) >= 0.8, "CVD بدأ يميل للاتجاه")
    add(18, sign * float(base["real_delta"]) >= 1.2, "الدلتا الحقيقية بدأت تتحول")
    add(14, sign * float(base["orderbook"]) >= 2.5, "دفتر الأوامر يميل للاتجاه")
    add(10, float(base["compression"]) >= 38, "السعر ما زال داخل ضغط")
    add(10, sign * float(micro["delta"]) >= 1.0, "تدفق الفريم السريع يدعم الاتجاه")
    add(10, float(micro["volume_ratio"]) >= 1.05, "الحجم بدأ يرتفع مبكرًا")
    return clamp(score), reasons


def early_stage_decision(
    direction: str,
    base: dict[str, float | bool],
    micro: dict[str, float | bool],
    previous: dict[str, Any] | None,
) -> tuple[str | None, float, list[str], str]:
    score, reasons = intent_score(direction, base, micro)
    sign = 1 if direction == "BUY" else -1
    micro_break = bool(micro["micro_break_up"] if direction == "BUY" else micro["micro_break_down"])
    extension = float(micro["extension_up"] if direction == "BUY" else micro["extension_down"])
    body = sign * float(micro["body_atr"])
    move = sign * float(micro["move_atr"])
    delta = sign * float(micro["delta"])

    # فاتت الحركة: لا استعداد ولا دخول.
    if extension > MAX_ENTRY_EXTENSION_ATR or body > 1.10 or move > 1.45:
        return None, score, reasons, f"فاتت الحركة: امتداد {extension:.2f} ATR"

    sent_entry = bool(previous and previous.get("sent_entry"))
    sent_early = bool(previous and previous.get("sent_early"))

    # أول كسر صغير على 5m/15m، بدون انتظار اكتمال شمعة 1H/4H.
    if (
        not sent_entry
        and score >= ENTRY_FLOW_THRESHOLD
        and micro_break
        and extension <= MAX_ENTRY_EXTENSION_ATR
        and delta >= 0.5
        and float(micro["volume_ratio"]) >= 1.02
    ):
        return "ENTRY", score, [
            "أول كسر على فريم التوقيت الداخلي",
            f"امتداد مبكر {extension:.2f} ATR",
            *reasons,
        ][:7], ""

    # استعداد قبل الكسر، ويجب ألا تكون الحركة ممتدة.
    if (
        not sent_early
        and score >= INTENT_THRESHOLD
        and not micro_break
        and extension <= MAX_EARLY_EXTENSION_ATR
    ):
        return "EARLY", score, reasons[:7], ""

    # الانفجار تأكيد بعد الدخول فقط.
    if (
        sent_entry
        and score >= EXPLOSION_FLOW_THRESHOLD
        and micro_break
        and move >= 0.35
        and extension <= 1.00
    ):
        return "EXPLOSION", score, ["استمرار التدفقات بعد الدخول", *reasons][:7], ""

    return None, score, reasons, "لا انتقال جديد"
...
class Engine:
    ...
    async def start(self) -> None:
        ...
        if SEND_STARTUP_MESSAGE and BOT_TOKEN and CHAT_ID:
            await send_telegram(
                self.telegram,
                "✅ <b>Ahmed Early Explosion Trader v3.0 EARLY-FLOW بدأ العمل</b>\n\n"
                f"⏰ الفريمات: {' / '.join(x.upper() for x in TIMEFRAMES)}\n"
                "🟡 استعداد مبكر جدًا\n🔵 دخول الآن\n🔥 بداية الانفجار\n⚠️ ضعف الزخم\n\n"
                "⚡ يستخدم 5M لتوقيت 1H و15M لتوقيت 4H داخليًا فقط.\n"
                ...
                try:
                    ...
                    timing_tasks = {
                        tf: asyncio.create_task(
                            self.client.klines(symbol, TIMING_INTERVALS[tf], 80)
                        ) for tf in TIMEFRAMES
                    }
                    oi_tasks = {
                    ...
                    for tf in TIMEFRAMES:
                        ...
                        timing_rows = await timing_tasks[tf]
                        oi_rows = await oi_tasks[tf]
                        if len(rows) < 210 or len(timing_rows) < 30:
                            continue

                        cf = candle_features(rows)
                        mf = micro_flow_features(timing_rows)
                        oi_values = [
                            float(x.get("sumOpenInterestValue", 0) or 0)
                            for x in oi_rows
                        ]
                        oi_change = pct(oi_values[-1], oi_values[-3]) if len(oi_values) >= 3 else 0.0
                        common = {
                            **cf,
                            "oi_change": oi_change,
                        ...
                        buy_score, _, _ = weighted_score("BUY", common)
                        sell_score, _, _ = weighted_score("SELL", common)
                        analyses[tf] = {
                            "rows": rows, "timing_rows": timing_rows,
                            "cf": cf, "mf": mf, "common": common,
                            "oi_change": oi_change,
                    ...
                    # اتجاه موحد للعملة: 1H أسرع، و4H يعطي السياق.
                    combined_buy = analyses["1h"]["buy_score"] * 0.62 + analyses["4h"]["buy_score"] * 0.38
                    combined_sell = analyses["1h"]["sell_score"] * 0.62 + analyses["4h"]["sell_score"] * 0.38
                    direction = "BUY" if combined_buy >= combined_sell else "SELL"
                    direction_gap = abs(combined_buy - combined_sell)
                    if direction_gap < 7.0:
                        await record_rejection(symbol, "1h/4h", direction, None, max(combined_buy, combined_sell), "الاتجاه غير محسوم")
                        return output
                    ...
                    for tf in TIMEFRAMES:
                        ...
                        mf = item["mf"]
                        common = item["common"]
                        state = await get_alert_state(symbol, tf)
                        stage, score, reasons, reject_reason = early_stage_decision(
                            direction, common, mf, state
                        )
                        await update_quiet_state(symbol, tf, score)
                        if not stage:
                            if reject_reason.startswith("فاتت"):
                                await record_rejection(symbol, tf, direction, None, score, reject_reason)
                            continue

                        # لا نرسل اتجاهًا معاكسًا لدورة مفتوحة.
                        if state and state.get("direction") != direction:
                            await record_rejection(symbol, tf, direction, stage, score, "يوجد اتجاه معاكس مفتوح لنفس الحركة")
                            continue

                        if not await claim_stage_once(
                            symbol, tf, direction, stage, score,
                            price=float(cf["price"]), atr_value=float(cf["atr"]),
                        ):
                            continue
                        ...
                        opposite = combined_sell if direction == "BUY" else combined_buy
                        sig = Signal(
                            symbol=symbol,
                            timeframe=tf,
                            direction=direction,
                            ...
                            score_change=0.0,
                            active_count=len(reasons),
                            pressure_score=float(cf["compression"]),
                            volume_acceleration=float(mf["volume_accel"]),
                            price_acceleration=float(mf["move_atr"]),
                            ...
                            breakout=bool(mf["micro_break_up"] if direction == "BUY" else mf["micro_break_down"]),
                            ...
                            reasons=[f"توقيت داخلي {TIMING_INTERVALS[tf].upper()}", *reasons][:7],
...
async def health():
    return {
        ...
        "service": "Ahmed Early Explosion Trader v3.0 EARLY-FLOW",
...
<body><div class="wrap">
<h1>Ahmed Early Explosion Trader v3.0 EARLY-FLOW</h1>
