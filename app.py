...
class Engine:
    ...
    async def scan(self) -> tuple[int, int]:
            ...
            async with sem:
                try:
                    ...
                    for tf in TIMEFRAMES:
                        ...
                        oi_values = [float(x.get("sumOpenInterestValue", 0) or 0) for x in oi_rows]
                        ...
                        sig = Signal(
                            ...
                            breakout=bool(cf["breakout_up"] if direction == "BUY" else cf["breakout_down"]),
        ...
        alerts.sort(key=lambda x: (stage_rank(x.stage), x.score, x.score_change), reverse=True)
        sent = 0
...
<a href="/stats">stats</a> · <a href="/checkpoints">checkpoints</a> ·
