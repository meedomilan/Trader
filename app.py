...
def evaluate_symbol_trades(symbol, price, ts=None):
    ...
    for aid in ids:
        ...
        if not tr.get('sl_hit_ts') and _crossed(side,price,sl,'SL'):
            ...
            if not first_hit: fields['first_hit']='SL'; fields['first_hit_ts']=ts
...
def result_stats():
    ...
    return {'total':total,'open':open_n,'evaluated':evaluated,'wins':wins,'sl_first':sl_first,'tp1':tp1,'tp2':tp2,'tp3':tp3,'win_rate':(wins/evaluated*100 if evaluated else 0),'strategies':[dict(r) for r in strategy_rows],'groups':[dict(r) for r in rows],'recent':[dict(r) for r in recent]}
...
def health():
    ...
    return jsonify({"ok":True,"version":state["version"],"ws_connected":state["ws_connected"],"ws_last_event":state["ws_last_event"],"radar_count":state["radar_count"],"bootstrapped":state["bootstrapped"],"bootstrap_total":state["bootstrap_total"],"active_trades":len(active_trades),"database":DB_PATH,"rest_backoff_seconds":blocked,"last_error":state["last_error"]})
