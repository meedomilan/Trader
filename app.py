...
state = {
    "version": "1.15 SETUP ASSISTED ENTRY GATE",
...
def evaluate_waiting_confirmations(symbol):
    ...
    for key,p in items:
        ...
        # Fast/live confirmation is allowed only after the independent trader entry gate opens.
        ...
        if res and res.get("confirmed"):
            ...
            s["reasons"]=list(s.get("reasons",[]))+[
                ...
                ("أول تأكيد حي بعد فتح بوابة الدخول" if res.get("fast") else ("إعادة اختبار مصغرة + استرداد" if res.get("mode")=="micro retest + reclaim" else "ثبات زخم مباشر بعد الكسر")),
                "الدخول تم بعد أن أصبح موقع الصفقة مناسبًا",
...
def cooldown_key(symbol,s=None,combo=None):
    # One Telegram entry per symbol during the symbol cooldown window.
    # Multiple strategies for the same coin are merged into one message.
    return f"SYMBOL|{symbol}"


def can_send(key):
    return time.time()-state["last_alerts"].get(key,0) >= SYMBOL_COOLDOWN_MIN*60
...
def _setup_entry_gate(ctx, sig):
    ...
    if rr < 0.60:
        return False, "مساحة الهدف ضعيفة جدًا"
...
def process_signals(symbol, signals, budget):
    """Send at most ONE Telegram entry per symbol.

    If several strategies confirm on the same coin/side, combine them into one alert.
    If BUY and SELL both confirm, take only the higher-scoring side.
    """
    ...
    for sig in signals:
        ...
        if performance_allows_send(sig):
            sig["score"]=(
                float(sig.get("quality",0))
                + min(10,len(sig.get("helpers",[]))*1.5)
                + float(sig.get("historical_boost",0.0))
                + min(8,max(0,float(sig.get("context_score",50))-50)*0.20)
            )
            allowed.append(sig)
        else:
            with state_lock:
    ...
    # Choose the stronger DIRECTION first, so contradictory strategy alerts do not appear.
    side_groups={}
    for side in ("BUY","SELL"):
        g=[x for x in allowed if x["side"]==side]
        if g:
            g=sorted(g,key=lambda x:x.get("score",0),reverse=True)
            side_groups[side]=(g[0].get("score",0),g)
    if not side_groups:
        return 0

    chosen_side=max(side_groups.items(), key=lambda kv: kv[1][0])[0]
    group=side_groups[chosen_side][1]

    # One instance per strategy, preserving the best timeframe/score for that strategy.
    uniq=[]
    seen=set()
    for x in group:
        if x["strategy"] not in seen:
            uniq.append(x)
            seen.add(x["strategy"])
    ...
    combo=None
    if len(uniq)>=2:
        combo=[x["strategy"] for x in uniq]
        # Use the safest structural stop among confirmed strategies.
        lead["sl"]=min(x["sl"] for x in uniq) if chosen_side=="BUY" else max(x["sl"] for x in uniq)
        risk=abs(lead["entry"]-lead["sl"])
        lead["tp"]=[lead["entry"]+(risk*r if chosen_side=="BUY" else -risk*r) for r in (1,2,3)]
        lead["quality"]=min(99,max(x["quality"] for x in uniq)+min(12,3*(len(uniq)-1)))
        lead["reasons"]=[f"توافق {len(uniq)} استراتيجيات على نفس العملة"] + list(dict.fromkeys(
            r for x in uniq[:3] for r in x.get("reasons",[]) if
            not str(r).startswith("قرار المحلل قبل الاستراتيجية:")
            and not str(r).startswith("خطة التداول:")
        ))[:10]

    oi=get_oi_soft(symbol)
    helpers_list=list(dict.fromkeys(h for x in uniq for h in x.get("helpers",[])))[:8]
    funding=lead.get("funding")

    if tg(alert_text(symbol,lead,helpers_list,oi,funding,combo)):
        persist_alert(symbol,lead,helpers_list,oi,funding,combo)
        record(key,lead,combo)
        return 1
    return 0
...
def health():
    ...
    return jsonify({"ok":True,"version":state["version"],"ws_connected":state["ws_connected"],"ws_last_event":state["ws_last_event"],"radar_count":state["radar_count"],"bootstrapped":state["bootstrapped"],"bootstrap_total":state["bootstrap_total"],"active_trades":len(active_trades),"waiting_confirmation":len(pending_confirmations),"post_sl_tracking":len(post_sl_trades),"database":DB_PATH,"rest_backoff_seconds":blocked,"setups_detected":state.get("setups_detected",0),"setups_waiting_entry":state.get("setups_waiting_entry",0),"entry_gates_opened":state.get("entry_gates_opened",0),"live_confirmed":state.get("live_confirmed",0),"last_error":state["last_error"]})
