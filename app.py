...
state = {
    "version": "1.14 DIVERSIFIED SYMBOL COOLDOWN",
...
def evaluate_waiting_confirmations(symbol):
    ...
    for key,p in items:
        ...
        gate_open=bool(latest_ctx.get("tradeable")) and latest_ctx.get("decision")==side
        if not gate_open:
            with pending_lock:
                live=pending_confirmations.get(key)
                if live is not None:
                    live["entry_gate_open"]=False
                    live["waiting_reason"]=latest_ctx.get("location","انتظار منطقة دخول مناسبة")
...
def analyze_symbol(symbol):
    ...
    # 1) Analyze the coin independently first. This controls ENTRY, not setup discovery.
    ctx=build_coin_context(symbol,dfs)

    # 2) ALWAYS scan the real strategy detectors, even when trader decision is WAIT.
    # WAIT means "do not enter now", never "there is no setup".
    candidates=[]
    ...
    # 3) Every genuine setup is preserved. Coin analysis only decides whether the ENTRY GATE is open.
    signals=[]; funding=funding_cache.get(symbol)
    for sig in candidates:
        if sig is None: continue

        # Attach the current trader analysis even if it says WAIT.
        side=sig["side"]
        own=float(ctx.get("buy_score" if side=="BUY" else "sell_score",0))
        opp=float(ctx.get("sell_score" if side=="BUY" else "buy_score",0))
        attach_coin_context(sig,ctx)

        gate_open=bool(ctx.get("tradeable")) and ctx.get("decision")==side
        sig["entry_gate_open"]=gate_open
        ...
        # Priority still favors context agreement, but WAIT setups are NOT discarded.
        context_bonus=min(8,max(0,(own-50))*0.20)
        if not gate_open:
            context_bonus=min(context_bonus,2.0)
            sig["reasons"]=list(sig.get("reasons",[]))+[
                "الاستراتيجية موجودة فعليًا لكن الدخول غير مناسب الآن",
                f"بوابة الدخول: {ctx.get('location','انتظار')}"
            ]
